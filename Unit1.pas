...
uses
  ...
  Dialogs, Math, StdCtrls, ExtCtrls, Buttons, ComCtrls;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure DrawCurve(n2, n3: Complex; rough1, rough2, d2, th_start, th_end: Extended; n, m, Ox, Oy, isLg: Integer);
    procedure Curve;
...
function CAbs(c1: Complex): Extended;
begin
  Result := sqrt(c1.a*c1.a + c1.b*c1.b);
end;
...
begin
  k.a := 2 * Pi / 0.1542;
  k.b := 0;
  k1.a := 2 * Pi / 0.1542 * sin(theta1);
  k1.b := 0;
  I1.a := 1;
  I1.b := 0;
  Crough1.a := -2*rough1*rough1;
  Crough1.b := 0;
  Crough2.a := -2*rough2*rough2;
  Crough2.b := 0;
  Cd2.a :=0;
  Cd2.b := 2*d2;
  //showmessage(floattostr(cd2.a));
  //ttt.a := 0; ttt.b:=0;
  ctheta1.a := cos(theta1);
  ctheta1.b := 0;
  ...
  //k2 := k1;
  //k2 := sqrt(1-ctheta)
  k2 := CMultiply(k, CSqrt(CMinus(CMultiply(n2, n2), CMultiply(ctheta1, ctheta1))));
  //showmessage(floattostr(cabs(CMinus(CMultiply(n2, n2), CMultiply(ctheta2, ctheta2)))));
  k3 := CMultiply(k, CSqrt(CMinus(CMultiply(n3, n3), CMultiply(ctheta1, ctheta1))));
  //if ctheta2.a > 1 then begin k2.a:=0;k2.b:=0; end;
  //showmessage(floattostr(cabs(k2)/k1.a));
  r12 := CDivide(CMinus(k1, k2), CPlus(k1, k2));
  //showmessage(floattostr(cabs(r12)));
  r23 := CDivide(CMinus(k2, k3), CPlus(k2, k3));
  //showmessage(floattostr(cabs(r12))+chr(13)+chr(10)+floattostr(cabs(r23)));
  //r23.a := 1; r23.b := 0;               
  //showmessage(floattostr(r23.a));
  r_12 := CMultiply(r12, CExp(CMultiply(CMultiply(k1, k2),Crough1)));
  //showmessage(floattostr(CExp(CMultiply(CMultiply(k1, k2),Crough1)).a));
  r_23 := CMultiply(r23, CExp(CMultiply(CMultiply(k2, k3),Crough2)));
  //k2.b :=0;
  //Ctt.a := 1;
  //ctt.b := 0;
  CTT := CExp(CMultiply(k2, Cd2));
  ...
  Result := CDivide(CPlus(r_12, CMultiply(r_23, CTT)),CPlus(I1, CMultiply(r_12, CMultiply(r_23, CTT))));
  //showmessage(floattostr((cabs(r12)+cabs(r23))/(1+cabs(r12)*cabs(r23))));
end;

procedure TForm1.DrawCurve(n2, n3: Complex; rough1, rough2, d2, th_start, th_end: Extended; n, m, Ox, Oy, isLg: Integer);
var
  i: Integer;
  th, R, RN: Extended;
  //xx: Complex;
begin
  ...
  if isLg = 0 then begin
    for i := trunc(n*th_start) + 1 to trunc(n*th_end) do
    begin
      th := i/n;
      R := CAbs(ComputeX(n2, n3, rough1, rough2, d2, th/180*pi)) / RN;
      Image1.Canvas.LineTo(trunc((i/n-th_start)*500/(th_end-th_start))+Ox, Oy-trunc(log10(R)*m));
    ...
    begin
      th := i/n;
      R := CAbs(ComputeX(n2, n3, rough1, rough2, d2, th/180*pi)) / RN;
      Image1.Canvas.LineTo(trunc((i/n-th_start)*500/(th_end-th_start))+Ox, Oy+m-trunc(R*R*m));
      //showmessage(floattostr(R));
    end;
  end;
end;
...
begin
  ...
  Form3.ScrollBar2.Position := trunc((strtofloat(edit9.Text) - strtofloat(edit10.Text)) * 100);
end;
